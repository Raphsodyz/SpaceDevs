...
using static Cross.Cutting.Helper.Constantes;
...
namespace Infrastructure.Persistence.Repository
{
    public class GenericRepository<T> : IGenericRepository<T> where T : EntidadeBase
    {
        ...
        public async Task<Paginacao<T>> GetAllPaged(int page, int pageSize,
            IEnumerable<Expression<Func<T, bool>>> filters = null,
            Expression<Func<IQueryable<T>, IOrderedQueryable<T>>> orderBy = null,
            string includedProperties = "")
        {
            ...
            if (totalPages < page) throw new InvalidOperationException($"{MensagensErro.PaginaInvalidaSelecionada} Total pages = {totalPages}");
            if (orderBy != null)
            {
            ...
            var result = new Paginacao<T>(results, totalPages, page, entityCount);
            return result;
        }
